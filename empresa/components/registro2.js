...
export default function registro2({ navigation, route }) {
    let [fontsLoaded] = useFonts({
        NotoSans_400Regular,
        Mukta_400Regular,
    });
    const CrearEmpresa = async (bio,contra,foto,nombre,correo) => {
        const login = await fetch(`http://stw-uvg.site:3186/empresa`, {
            method: 'POST',
            headers: {
    ...
    const Verificar = (foto, bio) => {
        if (foto === null || bio === null) {
            ...
            //(bio,contra,foto,nombre,correo)
            CrearEmpresa(bio,route.params.contra,foto,route.params.nombre,route.params.correo)
            //navigation.navigate('Login')
        }
    }
